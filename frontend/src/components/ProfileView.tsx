...
export const ProfileView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn relative">
      ...
      {/* Profile Card Banner */}
      <div className="glass-card p-6 sm:p-8 space-y-6 border border-slate-200/80 dark:border-white/10 relative overflow-hidden">
        
        {/* Top Cover Gradient Accent (Electric/Royal Blue) */}
        <div className="h-32 -mx-8 -mt-8 bg-gradient-to-r from-[#2563EB]/25 via-[#60A5FA]/20 to-[#93C5FD]/20 border-b border-slate-200/50" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-18">
          <div className="flex items-end gap-4">
            <img 
              ...
              className="w-24 h-24 sm:w-26 sm:h-26 rounded-full object-cover border-4 border-white dark:border-[#0e1524] shadow-md"
            />
            <div className="pb-1">
          ...
          {isOwnProfile ? (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-full bg-[#F8FAFC] dark:bg-white/10 text-slate-700 dark:text-gray-200 hover:bg-slate-100 font-bold text-xs flex items-center gap-1.5 border border-slate-200 dark:border-white/10 transition-colors shadow-2xs"
              >
                <Settings className="w-4 h-4 text-[#2563EB]" /> Settings
              </button>
            </div>
          ) : (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm ${
                selectedProfileUser.isFollowing
                  ? 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-gray-200 border border-slate-200 hover:bg-red-50 hover:text-red-600'
        ...
        {/* Bio */}
        <p className="text-sm text-slate-700 dark:text-gray-300 leading-relaxed max-w-2xl font-medium">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1 font-mono">
          ...
          <div className="p-3.5 rounded-2xl bg-[#F8FAFC] dark:bg-[#131c30] border border-slate-200/80 dark:border-white/10">
      ...
      {/* Profile Tabs */}
      <div className="flex border-b border-slate-200 dark:border-white/10 text-xs sm:text-sm font-bold overflow-x-auto gap-2">
        {[
      ...
      {profileTab === 'posts' && (
        <div className="space-y-4">
          {userPosts.length === 0 ? (
            <div className="glass-card p-10 text-center text-slate-400 text-sm font-medium">
      ...
      {profileTab === 'stats' && (
        <div className="glass-card p-6 space-y-4 font-mono">
          <h3 className="font-black text-slate-900 dark:text-white text-base flex items-center gap-2">
            <PieChart className="w-5 h-5 text-[#2563EB]" /> Historical Trading Metrics & Risk Index
          </h3>
          
          <div className="space-y-3 text-xs text-slate-700 dark:text-gray-300">
            <div className="flex justify-between border-b border-slate-100 dark:border-white/10 pb-2.5">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-[#2563EB]">Strict Stop-Loss Verified</span>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="relative w-full max-w-md sm:w-96 h-full bg-white dark:bg-[#0e1524] shadow-2xl border-l border-slate-200 dark:border-white/10 p-6 flex flex-col justify-between overflow-y-auto z-10 animate-slideInRight">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/10 pb-4">
                <div>
                  <h3 className="font-black text-slate-900 dark:text-white text-base flex items-center gap-2">
                    <Settings className="w-5 h-5 text-[#2563EB]" /> App Settings & Preferences
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">Customize theme, language & notifications</p>
                </div>
                <button
                  onClick={() => setIsSettingsDrawerOpen(false)}
                  className="p-1.5 rounded-full bg-slate-100 dark:bg-white/10 text-slate-400 hover:text-slate-900 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left transition-all flex items-center justify-between ${
                      theme === 'dark'
                        ? 'border-[#2563EB] bg-blue-500/15 text-white font-bold shadow-xs'
                  ...
                  >
                    ...
                    {theme === 'dark' && <span className="text-[#2563EB] font-bold text-xs">✓</span>}
                  </button>
              ...
              <div className="space-y-3 pt-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block flex items-center gap-1.5">
                  <Globe className="w-4 h-4 text-[#2563EB]" /> 2. Language / भाषा
                </label>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`p-3 rounded-2xl border text-left transition-all ${
                        language === lang.id
                          ? 'border-[#2563EB] bg-blue-50/70 text-[#2563EB] font-bold'
              ...
              <div className="space-y-3 pt-2">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block flex items-center gap-1.5">
                  <Bell className="w-4 h-4 text-[#2563EB]" /> 3. Notifications
                </label>
                <div className="space-y-2">
                  
                  <div className="p-3.5 rounded-2xl bg-[#F8FAFC] dark:bg-white/5 border border-slate-200/80 dark:border-white/10 flex items-center justify-between">
                    <div>
            ...
            {isOwnProfile && (
              <div className="pt-4 border-t border-slate-100 dark:border-white/10 space-y-2">
                <button
                  onClick={logout}
                  className="w-full py-3 rounded-full bg-red-50 border border-red-200 text-red-600 font-bold text-xs hover:bg-red-100 transition-all flex items-center justify-center gap-2"
                >
