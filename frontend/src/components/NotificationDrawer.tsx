        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Social interactions, follower alerts & price breakouts.</p>
            </div>
          </div>

          <button
            onClick={markNotificationsAsRead}
            className="flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold bg-[#F8FAFC] dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:text-[#2563EB] dark:hover:text-[#60A5FA] hover:border-slate-300 dark:hover:border-slate-600 transition-all shadow-2xs"
          >
            <CheckCheck className="w-4 h-4 text-[#2563EB] dark:text-[#60A5FA]" /> Mark All Read
          </button>
        </div>
            ...
            filtered.map(n => (
              <div 
                ...
                className={`p-4 rounded-2xl border transition-all flex items-start justify-between gap-4 shadow-2xs ${
                  n.read 
                    ? 'bg-[#F8FAFC] dark:bg-[#1E293B] border-slate-200/80 dark:border-slate-800' 
                    : 'bg-white dark:bg-[#1E293B] border-blue-200 dark:border-blue-500/40 shadow-xs'
                }`}
              >
                <div className="flex items-start gap-3">
                  {n.user ? (
                    <img src={n.user.avatar} alt="" className="w-10 h-10 rounded-full object-cover border border-slate-200 dark:border-slate-700 shrink-0" />
                  ) : (
                    <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 text-[#2563EB] dark:text-[#60A5FA] flex items-center justify-center shrink-0">
                ...
                {n.targetStockSymbol && (
                  <button
                    ...
                    className="px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 text-xs font-mono font-bold text-[#2563EB] dark:text-[#60A5FA] hover:bg-[#2563EB] hover:text-white transition-colors shrink-0 shadow-2xs"
                  >
                    ${n.targetStockSymbol}
                  </button>
                )}
