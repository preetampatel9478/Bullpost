...
export const StockDetailModal: React.FC = () => {
  ...
  return (
    <div className="modal-overlay">
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-[32px] w-full max-w-3xl max-h-[92vh] overflow-y-auto p-6 sm:p-7 shadow-2xl animate-fadeIn text-slate-900 dark:text-white space-y-6">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-900/30 text-[#2563EB] dark:text-[#60A5FA]">
                {selectedStock.name}
              </span>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-4.5 rounded-2xl bg-[#F8FAFC] dark:bg-[#1E293B] border border-slate-200/80 dark:border-slate-800">
          <div>
          ...
          <div className="flex items-center gap-6 font-mono text-xs text-slate-700 dark:text-slate-300">
            <div>
              <span className="text-[10px] text-slate-400 block uppercase font-sans font-bold">DAY HIGH</span>
              <span className="font-bold text-slate-900 dark:text-slate-200">₹{selectedStock.dayHigh}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-400 block uppercase font-sans font-bold">DAY LOW</span>
              <span className="font-bold text-slate-900 dark:text-slate-200">₹{selectedStock.dayLow}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-400 block uppercase font-sans font-bold">24H VOLUME</span>
              <span className="font-bold text-slate-900 dark:text-slate-200">{selectedStock.volume}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs font-bold text-slate-400">
            <span className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300">
              <BarChart2 className="w-4 h-4 text-[#2563EB] dark:text-[#60A5FA]" /> Interactive Technical Chart
            </span>
            <span className="font-mono text-[#2563EB] dark:text-[#60A5FA] font-bold">{selectedStock.bullishPercentage}% Trader Bullish Sentiment</span>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-blue-50/60 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-500/30">
          <div>
            <h4 className="font-bold text-sm text-slate-900 dark:text-white">Trading ${selectedStock.symbol}?</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400">Share your entry, target price, or verified P&L proof with the community.</p>
          </div>
          <button
            onClick={() => {
        ...
        <div className="space-y-4 pt-2">
          <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
            <Flame className="w-5 h-5 text-[#2563EB] dark:text-[#60A5FA]" /> Community Trader Discussions for ${selectedStock.symbol} ({stockPosts.length})
          </h3>

          {stockPosts.length === 0 ? (
            <div className="p-8 text-center glass-card text-slate-400 text-sm font-medium border border-slate-200 dark:border-slate-800">
