...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 dark:bg-[#111827]/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors">
      
      {/* 1. Live Market Ticker Marquee Bar */}
      <div className="bg-slate-900 dark:bg-[#070A12] text-white py-1.5 px-3 text-[11px] sm:text-xs border-b border-slate-800/60">
          ...
          <div className="marquee-content font-mono">
            {tickerPrices.concat(tickerPrices).map((stk, idx) => {
              ...
              return (
                ...
                >
                  <span className="font-bold text-[#60A5FA]">${stk.symbol}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <div className="flex-1 max-w-2xl relative" ref={searchRef}>
          <div className="relative flex items-center">
            <Search className="w-4 h-4 text-slate-400 dark:text-slate-400 absolute left-4 pointer-events-none" />
            <input
              type="text"
              ...
              className="w-full bg-[#F8FAFC] dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs sm:text-sm rounded-full pl-11 pr-10 py-2.5 focus:outline-none focus:border-[#2563EB] dark:focus:border-[#60A5FA] focus:ring-2 focus:ring-[#2563EB]/15 transition-all font-medium placeholder:text-slate-400 dark:placeholder:text-slate-400 shadow-xs"
            />
            {searchQuery && (
              <button 
                onClick={() => setSearchQuery('')}
                className="absolute right-3.5 text-slate-400 hover:text-slate-700 dark:hover:text-white p-0.5 rounded-full"
              >
                <X className="w-4 h-4" />
              </button>
            )}
          ...
          {isSearchFocused && searchQuery.trim().length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl overflow-hidden z-50 animate-fadeIn max-h-[80vh] overflow-y-auto">
              
              {matchingUsers.length > 0 && (
                <div className="p-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-2 flex items-center gap-1">
                    <UserIcon className="w-3.5 h-3.5 text-[#2563EB] dark:text-[#60A5FA]" /> Traders ({matchingUsers.length})
                  </div>
                  <div className="space-y-1">
                    {matchingUsers.map(user => (
                      <div
                        ...
                        className="flex items-center justify-between p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-[#1E293B] cursor-pointer transition-colors"
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="flex items-center gap-1 text-xs font-bold text-slate-900 dark:text-white">
                              {user.name}
                              {user.verified && <span className="text-[#2563EB] dark:text-[#60A5FA] text-[10px]">✓</span>}
                            </div>
              ...
              {matchingStocks.length > 0 && (
                <div className="p-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-2 flex items-center gap-1">
                    <TrendingUp className="w-3.5 h-3.5 text-[#2563EB] dark:text-[#60A5FA]" /> Stocks ({matchingStocks.length})
                  </div>
                  <div className="space-y-1">
                    {matchingStocks.map(stock => (
                      ...
                      >
                        <div>
                          <div className="text-xs font-bold text-slate-900 dark:text-white">${stock.symbol}</div>
                          <div className="text-[10px] text-slate-400">{stock.name}</div>
                        </div>
                        <div className="text-right font-mono">
                          <div className="text-xs font-bold text-slate-900 dark:text-white">₹{stock.currentPrice.toLocaleString('en-IN')}</div>
              ...
              {matchingHashtags.length > 0 && (
                <div className="p-3">
                  <div className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-2 flex items-center gap-1">
                    <Hash className="w-3.5 h-3.5 text-[#2563EB] dark:text-[#60A5FA]" /> Hashtags & Topics
                  </div>
                  <div className="space-y-1">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          {activeTab === 'profile' && (
            <button
              ...
              className="p-2.5 rounded-full bg-[#F8FAFC] dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-200 hover:text-[#2563EB] dark:hover:text-[#60A5FA] transition-all shadow-xs flex items-center justify-center group"
              title="App Settings"
            >
          ...
          <div className="relative shrink-0">
            <button
              ...
              className="p-2.5 rounded-full bg-[#F8FAFC] dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-200 hover:text-[#2563EB] dark:hover:text-[#60A5FA] hover:border-slate-300 dark:hover:border-slate-600 transition-all relative shadow-xs"
            >
            ...
            {showNotificationsPopover && (
              <div className="absolute right-0 mt-3 w-72 sm:w-80 md:w-96 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl p-4 z-50 animate-fadeIn">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5 mb-3">
                  <h4 className="font-bold text-xs sm:text-sm flex items-center gap-2 text-slate-900 dark:text-white">
                    <Bell className="w-3.5 h-3.5 text-[#2563EB] dark:text-[#60A5FA]" /> Notifications
                  </h4>
                  <button 
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {notifications.slice(0, 4).map(n => (
                    <div key={n.id} className="p-2.5 rounded-xl bg-slate-50 dark:bg-[#1E293B] text-[11px] flex gap-2.5 items-start">
                      {n.user ? (
                        <img src={n.user.avatar} alt="" className="w-7 h-7 rounded-full object-cover shrink-0" />
                      ) : (
                        <div className="w-7 h-7 rounded-full bg-blue-50 dark:bg-blue-900/30 text-[#2563EB] dark:text-[#60A5FA] flex items-center justify-center shrink-0">
